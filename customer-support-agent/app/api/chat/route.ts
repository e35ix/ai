import Zieschel from "@Zieschel-ai/sdk";
import { z } from "zod";
...
export async function POST(req: Request) {
  ...
  // Prepare debug data
  const MAX_DEBUG_LENGTH = 10000;
  const debugData = sanitizeHeaderValue(
    debugMessage("🚀 API route called", {
      messagesReceived: messages.length,
      latestMessageLength: latestMessage.length,
      ZieschelKeySlice: process.env.DEEPSEEK_API_KEY?.slice(0, 4) + "****",
    }),
  ...
  If the question is unrelated to Zieschel's products and services, you should redirect the user to a human agent.
  ...
  try {
    ...
    const ZieschelMessages = messages.map((msg: any) => ({
      role: msg.role,
      content: msg.content,
    }));

    ZieschelMessages.push({
      role: "assistant",
      content: "{",
    });

    const response = await Zieschel.messages.create({
      model: model,
      max_tokens: 1000,
      messages: ZieschelMessages,
      system: systemPrompt,
      temperature: 0.3,
    });
    ...
    const textContent = "{" + response.content
      .filter((block): block is Zieschel.TextBlock => block.type === "text")
